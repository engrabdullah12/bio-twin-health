...
import { FlaskConical, AlertTriangle, CheckCircle, Loader2, Search, History } from "lucide-react";
...
export default function DrugSimulator() {
  ...
  const saveSimulation = async (drugName: string, res: SimResult) => {
    if (!user) return;
    const { error } = await supabase.from("medication_history").insert({
      user_id: user.id,
      drug_name: drugName,
      compatibility_score: res.compatibility,
      risks: res.risks,
      benefits: res.benefits,
      interactions: res.interactions,
    });
    if (error) {
      toast({ title: "Error saving simulation", description: error.message, variant: "destructive" });
    } else {
      loadHistory();
    }
  };
  ...
  const runSimulation = (drug: string) => {
    ...
    setTimeout(() => {
      const res = mockResults[drug] || mockResults["Vitamin D3"];
      setResult(res);
      setSimulating(false);
      saveSimulation(drug, res);
    }, 2000);
  };
      ...
      <div className="flex items-center justify-between">
        ...
        {user && (
          <button
            ...
            className={`flex items-center gap-1.5 text-xs font-body px-3 py-1.5 rounded-full border transition-all ${
              showHistory ? "bg-primary/20 border-primary/40 text-primary" : "border-border text-muted-foreground hover:text-foreground"
            }`}
          >
      ...
      <AnimatePresence mode="wait">
        {showHistory ? (
          <motion.div key="history" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="space-y-3">
            {history.length === 0 ? (
              <p className="text-xs text-muted-foreground text-center py-4">No simulations yet</p>
            ) : (
              history.map((h) => (
                <div key={h.id} className="flex items-center justify-between p-3 bg-secondary/30 rounded-lg">
                  <div>
                    <p className="text-sm font-display font-medium text-foreground">{h.drug_name}</p>
                    <p className="text-xs text-muted-foreground font-body">
                      {new Date(h.simulated_at).toLocaleDateString()}
                    </p>
                  </div>
                  <span className={`text-lg font-display font-bold ${compatColor(h.compatibility_score)}`}>
          ...
          <motion.div key="simulator" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="space-y-5">
            {/* Search */}
